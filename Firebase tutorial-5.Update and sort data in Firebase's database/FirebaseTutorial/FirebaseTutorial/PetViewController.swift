...
//        (UIApplication.shared.delegate as! AppDelegate).fireBaseRef.child("pets").queryOrdered(byChild: "name").observe(.value, with: { snapshot in
        (UIApplication.shared.delegate as! AppDelegate).fireBaseRef.observe(.value, with: { snapshot in
            ...
            self.pets = [Pet]()
            for key in Array(dictPets.keys).sorted() {
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let editAction = UITableViewRowAction(style: .default,
                                              ...
                                              handler: { (action, indexPath) in
                                                print("Edit cell")
                                                var txtPetName: UITextField?
                                                var editingPet:Pet = self.pets[indexPath.row]
